...
void Display::write_digit(byte section, byte digit, bool dps_on)
{
  ...
  // set the section
  if (section == m_digit_to_blink && millis() - last_blinked > 250) // change epsilon
  {
  ...
  {
      digitalWrite(section_pins[i], i != section || digit == NO_DIGIT || (section == m_digit_to_blink && blinking_state == false)); // if i == section or this digit is turned off or this digit is blinking off, write low (false) 
  }
...
void Display::set_data(byte in_data[NUM_SECTIONS])
{
  for (int i = 0; i < NUM_SECTIONS; i++)
  {
    set_data_ind(i, in_data[i]);
  }
}
...
int Display::set_digit_to_blink(int d)
{
  m_digit_to_blink = d;
//  if (m_digit_to_blink == 0 && data[0] == NO_DIGIT)
//    m_digit_to_blink++;
  return m_digit_to_blink;
}

int Display::blink_right()
{
  m_digit_to_blink = (m_digit_to_blink + 1) % NUM_SECTIONS;
  if (m_digit_to_blink == 0 && data[0] == NO_DIGIT)
    m_digit_to_blink++;
  return m_digit_to_blink;
}
